...
#### CaseKeyPath Extraction Mode (Recommended)

For simple, direct forwarding where a parent Input case directly wraps a child Input:

```swift
// Reads as: "forward input childTapped TO child action"
@ForwardInput(\Input.Cases.childTapped, to: \Action.Cases.child)
```

This syntax works in two modes:

**Standalone (without `@ComposableEffectRunner`)**:
...
struct ParentFeature: StateMachine {
    ...
    var body: some Reducer<State, Action> {
        ...
        Self.childInputForwarder  // Generated by @ForwardInput
...
**Integrated (with `@ComposableEffectRunner`)**:
...
struct ParentFeature: StateMachine {
    ...
    // Forwarding is automatically baked into the generated body
}
```
...
```swift
@ForwardInput(\Input.Cases.child1Tapped, to: \Action.Cases.child1)
@ForwardInput(\Input.Cases.child2Tapped, to: \Action.Cases.child2)
struct ParentFeature: StateMachine {
    // ...
    
    var body: some Reducer<State, Action> {
        Scope(state: \.child1, action: \.child1) { Child1Feature() }
        Scope(state: \.child2, action: \.child2) { Child2Feature() }
        
        Self.child1InputForwarder  // Generated
        Self.child2InputForwarder  // Generated
...
#### Legacy Function-Based Mode

For complex mapping logic that can't be expressed as a simple case extraction, use the legacy function-based syntax. This requires `@ComposableEffectRunner` and the mapping function must be in the main body:
...
struct ParentFeature: StateMachine {
    ...
    static func forwardChildInput(_ input: Input) -> ChildFeature.Input? {
        switch input {
        case .childTapped(let childInput) where childInput.isValid:
            return childInput
        default:
            return nil
        }
